...
质量规范不依赖用户反复提醒，而是作为 Agent 默认工作流执行：

- **门禁/规则强制要求的修复**（`check` WARN/BLOCK、`rules` finding 等）：自动执行并汇报。
- **启发式质量优化**（拆分文件、引入新抽象/约定/字段、规范化、重构等）：默认不静默执行；先给出决策卡——优化点、认知点变化、风险与验证成本、选项（执行 / 暂缓并记录 / 不做），由用户决定。
- **每次任务结束固定输出质量优化清单**：门禁修复（自动）、用户批准的启发式优化、被暂缓/拒绝的优化（含原因）。
- **暂缓的优化必须记录**原因和回访条件，避免下次由用户再次提醒；有可复查结论才 `docs record`。
...
对改动范围运行相称的语言检查、测试和 OpenArch 验证。项目已配置 `quality_rules` 时运行 `openarch rules scan --check`；要观察事实目录健康度时可用 `openarch rules check --unused`（零消费者事实为 WARN 提示）；文档或经验改动按实际 `DocumentStore` 做相似检查。不要手工删除 `pending evidence` 或 `baseline` 分片来获得干净提交；先调查内容身份、可达性和协调边界。**回复结束时固定包含“质量优化清单”**，让用户对门禁修复、已批准优化、暂缓/拒绝优化有完整感知。
