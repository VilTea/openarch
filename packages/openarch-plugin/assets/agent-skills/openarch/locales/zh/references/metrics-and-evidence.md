...
`I_push = Σ λ_ast × α_struct × log2(inDegree + 1) × ω_layer × λ_joint` 是可解释的变更传播上界，不是 `LOC` 或质量分。
...
普通 `scan` 只更新观察到的 `P95`；封存的校准 epoch 仍是未变文件的分母。`scan --seal-calibration` 是经审查的团队动作。观察值与封存值偏离为 `CALIBRATION_SHIFT`，仅报告。
