...
`--docs-scope` takes **`<scopeId>=<relative-path>`** (e.g. `fund-claude-agent=agent`): the scopeId precedes `=`, the path within the document store follows it; a bare value fails validation.
