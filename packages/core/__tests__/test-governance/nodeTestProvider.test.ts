...
beforeAll(() => {
  ...
  writeFileSync(file, `
    ...
    function checkThrows(fn: () => void) { try { fn(); assert.fail("did not throw"); } catch { assert.ok(true); } }
    test("uses helper wrapper", () => { checkThrows(() => { throw new Error("x"); }); });
...
describe("nodeTestProvider", () => {
  it("extracts test-body metrics and node:test facts through AST queries", async () => {
    ...
    expect(result.tests.map((test) => test.name)).toEqual([
      "has assert.strictEqual", "has assert.ok", "has bare assert call", "skipped", "inner with deepStrictEqual", "missing", "calls imported subject", "uses helper wrapper", "counts only its own control flow",
    ]);
    ...
    // P3（2026-08-12 体验反馈）：同文件定义的断言包装函数（非前缀命名）不再误扫
    expect(result.tests.find((test) => test.name === "uses helper wrapper")?.assertionCount).toBe(1);
    expect(result.findings.some((finding) => finding.testName === "uses helper wrapper" && finding.kind === "missing_assertion")).toBe(false);
